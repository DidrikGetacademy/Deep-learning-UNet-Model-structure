...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x, g):
        ...
        out = x * psi
        train_logger.debug( f"[AttentionBlock] Output shape: {out.shape},"  f"psi min={psi.min().item():.4f}, psi max={psi.max().item():.4f}"  )
        return out
...
class MultiScaleDecoderBlock(nn.Module):
    ...
    def forward(self, x, skip):
        train_logger.debug( f"[MultiScaleDecoderBlock] Input shapes -> x: {x.shape}, skip: {skip.shape}" )

        x = self.up(x)
        if x.size(2) != skip.size(2) or x.size(3) != skip.size(3):
            x = F.interpolate(x, size=(skip.size(2), skip.size(3)), mode='bilinear', align_corners=False)
            train_logger.debug(f"[MultiScaleDecoderBlock] Resized upsampled x to: {x.shape}")
...
class UNet(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=1, features=[16, 32, 64, 128]):
        ...
        for idx, feature in enumerate(reversed_features):
            ...
            train_logger.debug( f"[UNet] Added decoder & attention block -> in:{input_channels}, out:{feature}" )
